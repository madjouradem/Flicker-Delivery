...
import 'package:delivery_app/core/calsses/UploadCtr.dart';
import 'package:delivery_app/core/calsses/services.dart';
import 'package:delivery_app/core/functions/handlingData.dart';
import 'package:delivery_app/pages/EditProfile/EditProfileData.dart';
...
class EditProfileCtr extends EditProfileCtrAbs {
  ...
  @override
  void onInit() {
    username.text = myServices.box.read('delivery')['delivery_name'];
  ...
  changeUsername() async {
    if (formstate2.currentState!.validate()) {
      if (username.text != myServices.box.read('delivery')['delivery_name']) {
        statusRequest = StatusRequest.loading;
        update();
        var response = await data.changeUsername(
            myServices.box.read('delivery')['delivery_id'], username.text);
        statusRequest = handlingData(response);
        if (statusRequest == StatusRequest.success) {
          if (response['status'] == 'success') {
            myServices.box.read('delivery')['delivery_name'] = username.text;
  ...
  changePassword() async {
    if (formstate.currentState!.validate()) {
      if (sha1.convert(utf8.encode(currentpassword.text)).toString() ==
          myServices.box.read('delivery')['delivery_password']) {
        statusRequest = StatusRequest.loading;
        update();
  ...
  changeImage({required ImageSource imageSource}) async {
    ...
    if (imageName != '') {
      ...
      var response = await data.changeImage(
          myServices.box.read('delivery')['delivery_id'], imageName);
      statusRequest = handlingData(response);
      if (statusRequest == StatusRequest.success) {
        if (response['status'] == 'success') {
